...
function log(agentName, command, cwd, line, isHeader = false) {
...
function runCommand(command, cwd, agentName) {
...
const server = new Server({ name: "command-runner-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "run_command",
            description: "Execute a shell command and stream output to the shared agent log at /tmp/q_agent_log.txt. All agents should use this tool for full cross-agent visibility.",
            inputSchema: {
                type: "object",
                properties: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const { command, cwd = process.cwd(), agent_name = "unknown" } = request.params.arguments;
