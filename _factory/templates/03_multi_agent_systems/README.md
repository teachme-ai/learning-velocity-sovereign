...
```bash
/tmp/genkit_env/bin/python3 set_a_{{ industry_slug }}/logic/swarm.py
```
...
genkit start -- /tmp/genkit_env/bin/python3 set_a_{{ industry_slug }}/logic/swarm.py
```

Open `http://localhost:4000` in your browser. Navigate to **Flows → {{ industry_slug }}_agent_swarm** to run the flow and inspect each agent step in the trace viewer.

> Replace `set_a_{{ industry_slug }}` with any of the other set directories (`set_b_{{ industry_slug }}`, `set_c_supply_chain`, `set_d_edtech`, `set_e_legal`) to run those swarms.
