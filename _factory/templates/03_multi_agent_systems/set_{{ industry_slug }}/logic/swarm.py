"""
Session 03 — Set A: {{ industry_name }} Multi-Agent Swarm
...
# ── Genkit Flow ───────────────────────────────────────────────────────────────
@ai.flow()
async def {{ industry_slug }}_agent_swarm(input_data: SwarmInput) -> SwarmOutput:
    print("Starting Corporate {{ industry_name }} Triple-Agent Swarm...")
...
async def main():
    input_file = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../../01_data_pipeline_automation/set_a_{{ industry_slug }}/data/flagged_expenses.csv")
    )
    ...
    result = await {{ industry_slug }}_agent_swarm(SwarmInput(csv_data=csv_data))

    output_dir = "/tmp/{{ industry_slug }}_output"
