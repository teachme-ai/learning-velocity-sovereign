...
and RAG retrievers (Session 04) across {{ industry_name }}, {{ industry_name }}, Supply Chain, EdTech, Legal.
...
LobeChat Plugin Endpoint:
    ...
    Body: { "domain": "{{ industry_slug }}", "query": "What are the expense limits?" }
"""
...
DOMAIN_CONFIG = {
    "{{ industry_slug }}": {
        "label": "Corporate {{ industry_name }}",
        "swarm_module": "03_multi_agent_systems.set_a_{{ industry_slug }}.logic.swarm",
        "swarm_flow": "{{ industry_slug }}_agent_swarm",
        "rag_module": "04_sovereign_knowledge_rag.set_a_{{ industry_slug }}.logic.ingest_and_query",
        "rag_flow": "{{ industry_slug }}_rag_flow",
        "swarm_input_field": "csv_data",
        "session01_output": "/tmp/{{ industry_slug }}_output/flagged_expenses.csv",
    },
    "{{ industry_slug }}": {
        "label": "{{ industry_name }} & HIPAA",
        "swarm_module": "03_multi_agent_systems.set_b_{{ industry_slug }}.logic.swarm",
        "swarm_flow": "{{ industry_slug }}_agent_swarm",
        "rag_module": "04_sovereign_knowledge_rag.set_b_{{ industry_slug }}.logic.ingest_and_query",
        "rag_flow": "{{ industry_slug }}_rag_flow",
        "swarm_input_field": "csv_data",
        "session01_output": "/tmp/{{ industry_slug }}_output/scrubbed_billing.csv",
    },
    "supply_chain": {
        ...
        try:
            ...
            for span in spans:
                ...
                if domain == "unknown":
                    ...
                    for d in ["{{ industry_slug }}", "{{ industry_slug }}", "supply_chain", "edtech", "legal"]:
                        # Look for domain signatures in the raw trace data (like "{{ industry_slug }}_agent_swarm")
...
class ChatRequest(BaseModel):
    domain: str = Field(
        description="Industry domain: {{ industry_slug }} | {{ industry_slug }} | supply_chain | edtech | legal",
        examples=["{{ industry_slug }}"],
    )
    query: str = Field(
