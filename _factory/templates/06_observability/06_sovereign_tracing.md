...
3. Submit a Swarm Query for Set B ({{ industry_name }}):
   > "Audit the logs using {{ industry_slug }} swarm mode."
   *(Under the hood, this routes your query to the `{{ industry_slug }}_agent_swarm`.)*
...
Below is the visual trace generated for the {{ industry_name }} swarm. It proves that the request was successfully passed sequentially through the Clinical Analyst, Medical Auditor, and Compliance Reporter.

![Session 06 Trace Visualization](../../assets/proof/session_06/{{ industry_slug }}_trace.svg)
