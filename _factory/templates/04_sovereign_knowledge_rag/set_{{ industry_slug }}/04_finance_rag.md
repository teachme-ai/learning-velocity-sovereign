...
```
Open **http://localhost:4000** → **Flows** → `{{ industry_slug }}_rag_flow` → run with any policy question and inspect the retrieved context chunks in the trace viewer.
...
ChromaDB (PersistentClient)
      │
      ▼
ai.define_retriever("{{ industry_slug }}_policy") → ai.retrieve()
      │ top-3 semantic chunks
      ▼
ai.generate(system + docs + query) → llama3.2:1b
      │
      ▼
Grounded Answer → /tmp/{{ industry_slug }}_output/rag_answer.md
```

### Genkit Retriever Pattern
```python
async def {{ industry_slug }}_retriever(request: RetrieverRequest, ctx: ActionRunContext):
...
ai.define_retriever(name="{{ industry_slug }}_policy", fn={{ industry_slug }}_retriever)

@ai.flow()
async def {{ industry_slug }}_rag_flow(input_data: RAGInput) -> RAGOutput:
    retriever_result = await ai.retrieve(
        query=Document.from_text(input_data.query),
        retriever="{{ industry_slug }}_policy"
    )
...
Answer saved to /tmp/{{ industry_slug }}_output/rag_answer.md
```
...
---
**[Back to Curriculum Hub](../../README.md) | [Previous Lab: Session 03](../../03_multi_agent_systems/set_a_{{ industry_slug }}/03_{{ industry_slug }}_swarm.md) | [Next Lab: Session 05](../../05_advanced_ui_lobechat/05_advanced_ui_lobechat.md)**
