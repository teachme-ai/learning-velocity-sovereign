"""
Session 03 — Set A: AI for Educators Multi-Agent Swarm
...
# ── Genkit Flow ───────────────────────────────────────────────────────────────
@ai.flow()
async def ai_for_educators_agent_swarm(input_data: SwarmInput) -> SwarmOutput:
    print("Starting Corporate AI for Educators Triple-Agent Swarm...")
...
async def main():
    input_file = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../../01_data_pipeline_automation/set_a_ai_for_educators/data/flagged_expenses.csv")
    )
    ...
    result = await ai_for_educators_agent_swarm(SwarmInput(csv_data=csv_data))

    output_dir = "/tmp/ai_for_educators_output"
