"""
Session 04 — Set A: AI for Educators Sovereign RAG
...
COLLECTION = "ai_for_educators_policy"
...
async def ai_for_educators_retriever(request: RetrieverRequest, ctx: ActionRunContext) -> RetrieverResponse:
...
async def ai_for_educators_rag_flow(input_data: RAGInput) -> RAGOutput:
    ...
    response = await ai.generate(
        system="You are a Corporate AI for Educators Policy expert. Answer the question strictly based on the provided context. Do not speculate.",
...
# ── Entrypoint ────────────────────────────────────────────────────────────────
async def main():
    print("\n--- AI for Educators Sovereign RAG Pipeline ---")
    await ingest()
    result = await ai_for_educators_rag_flow(RAGInput(query=QUERY))
    out_dir = Path("/tmp/ai_for_educators_output")
