"""
Session 04 — Set A: AI for Cyber-Security Sovereign RAG
...
COLLECTION = "ai_for_cyber-security_policy"
...
async def ai_for_cyber-security_retriever(request: RetrieverRequest, ctx: ActionRunContext) -> RetrieverResponse:
...
async def ai_for_cyber-security_rag_flow(input_data: RAGInput) -> RAGOutput:
    ...
    response = await ai.generate(
        system="You are a Corporate AI for Cyber-Security Policy expert. Answer the question strictly based on the provided context. Do not speculate.",
...
# ── Entrypoint ────────────────────────────────────────────────────────────────
async def main():
    print("\n--- AI for Cyber-Security Sovereign RAG Pipeline ---")
    await ingest()
    result = await ai_for_cyber-security_rag_flow(RAGInput(query=QUERY))
    out_dir = Path("/tmp/ai_for_cyber-security_output")
