...
```
Open **http://localhost:4000** → **Flows** → `ai_for_cyber-security_rag_flow` → run with any policy question and inspect the retrieved context chunks in the trace viewer.
...
ChromaDB (PersistentClient)
      │
      ▼
ai.define_retriever("ai_for_cyber-security_policy") → ai.retrieve()
      │ top-3 semantic chunks
      ▼
ai.generate(system + docs + query) → llama3.2:1b
      │
      ▼
Grounded Answer → /tmp/ai_for_cyber-security_output/rag_answer.md
```

### Genkit Retriever Pattern
```python
async def ai_for_cyber-security_retriever(request: RetrieverRequest, ctx: ActionRunContext):
...
ai.define_retriever(name="ai_for_cyber-security_policy", fn=ai_for_cyber-security_retriever)

@ai.flow()
async def ai_for_cyber-security_rag_flow(input_data: RAGInput) -> RAGOutput:
    retriever_result = await ai.retrieve(
        query=Document.from_text(input_data.query),
        retriever="ai_for_cyber-security_policy"
    )
...
Answer saved to /tmp/ai_for_cyber-security_output/rag_answer.md
```
...
---
**[Back to Curriculum Hub](../../README.md) | [Previous Lab: Session 03](../../03_multi_agent_systems/set_a_ai_for_cyber-security/03_ai_for_cyber-security_swarm.md) | [Next Lab: Session 05](../../05_advanced_ui_lobechat/05_advanced_ui_lobechat.md)**