...
```bash
/tmp/genkit_env/bin/python3 set_a_ai_for_healthcare/logic/swarm.py
```
...
genkit start -- /tmp/genkit_env/bin/python3 set_a_ai_for_healthcare/logic/swarm.py
```

Open `http://localhost:4000` in your browser. Navigate to **Flows → ai_for_healthcare_agent_swarm** to run the flow and inspect each agent step in the trace viewer.

> Replace `set_a_ai_for_healthcare` with any of the other set directories (`set_b_ai_for_healthcare`, `set_c_supply_chain`, `set_d_edtech`, `set_e_legal`) to run those swarms.
