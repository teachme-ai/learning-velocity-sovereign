...
### Overview
This lab introduces an enterprise-grade, three-agent orchestration pipeline using **Google Genkit** (Python SDK). A `Clinical Data Harmonizer`, `Clinical Review Analyst`, and `Treatment Pathway Orchestrator` operate sequentially — each agent consuming the previous agent's output — to produce a final Comprehensive Patient Care Summary from unstructured patient encounter data.
### Environment Setup
```bash
