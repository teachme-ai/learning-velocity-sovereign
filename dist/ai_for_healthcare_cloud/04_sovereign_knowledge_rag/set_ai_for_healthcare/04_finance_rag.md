...
### Overview
This lab establishes a **fully local, sovereign RAG pipeline**—a critical setup for maintaining stringent data privacy and operational control within healthcare environments. We'll leverage **Google Genkit** (Python) for orchestration, **ChromaDB** as our vector store, and **Ollama** for both embedding and generation tasks. The practical workflow involves ingesting **Clinical Practice Guidelines**, embedding these documents using `nomic-embed-text` to capture their semantic meaning, and then indexing them efficiently into ChromaDB. Subsequently, a **Genkit-native Retriever** will be deployed to fetch highly relevant contextual information, enabling the system to generate a grounded, cited answer using `llama3.2:1b`—essential for evidence-based responses in clinical or administrative support scenarios.
### Environment Setup
```bash
