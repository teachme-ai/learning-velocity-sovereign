# Session 04: Sovereign Knowledge RAG (AI for Global Finance)
...
```
Open **http://localhost:4000** → **Flows** → `ai_for_global_finance_rag_flow` → run with any policy question and inspect the retrieved context chunks in the trace viewer.
...
ChromaDB (PersistentClient)
      │
      ▼
ai.define_retriever("ai_for_global_finance_policy") → ai.retrieve()
      │ top-3 semantic chunks
      ▼
ai.generate(system + docs + query) → llama3.2:1b
      │
      ▼
Grounded Answer → /tmp/ai_for_global_finance_output/rag_answer.md
```
...
async def ai_for_global_finance_retriever(request: RetrieverRequest, ctx: ActionRunContext):
    embedding = ollama.embed_text(text=request.query.text)
    results = chroma_collection.query(query_embeddings=[embedding], n_results=3)
    return ai.retrieve(docs=[Document.from_text(d) for d in results["documents"][0]])

ai.define_retriever(name="ai_for_global_finance_policy", fn=ai_for_global_finance_retriever)

@ai.flow()
async def ai_for_global_finance_rag_flow(input_data: RAGInput) -> RAGOutput:
    retriever_result = await ai.retrieve(
        query=Document.from_text(input_data.query),
        retriever="ai_for_global_finance_policy"
    )
...
Answer saved to /tmp/ai_for_global_finance_output/rag_answer.md
```
...
---
**[Back to Curriculum Hub](../../README.md) | [Previous Lab: Session 03](../../03_multi_agent_systems/set_a_ai_for_global_finance/03_ai_for_gl # Limit context window**