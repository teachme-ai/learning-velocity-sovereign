...
### Overview
This lab introduces an enterprise-grade, three-agent orchestration pipeline using **Google Genkit** (Python SDK). A `Financial Analyst` and `Corporate Auditor` work sequentially in parallel, leveraging concurrent processing to analyze large datasets — each agent consuming the previous agent's output partially — to produce a final Corporate Investigation Memo from flagged expense data. The pipeline must efficiently handle high-throughput data ingestion while minimizing the risk of false positives, ensuring that only legitimate expenses are flagged for review.
### Environment Setup
```bash
