...
### Overview
This lab develops a fully local, sovereign RAG pipeline leveraging Google Genkit (Python), ChromaDB (vector store), and Ollama (embeddings + generation). The system ingests the Corporate Travel & Expense Policy, embeds it using `nomic-embed-text`, indexes it into ChromaDB, then utilizes a Genkit-native Retriever to retrieve relevant context and generate a grounded, cited answer using `llama3.2:1b`. By integrating these technologies within a secure, on-premises environment, this pipeline ensures compliance with organizational data governance policies while enabling efficient policy management and retrieval.
### Environment Setup
```bash
