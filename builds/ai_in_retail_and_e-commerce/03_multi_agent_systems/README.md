...
```bash
/tmp/genkit_env/bin/python3 set_a_ai_in_retail_and_e-commerce/logic/swarm.py
```
...
genkit start -- /tmp/genkit_env/bin/python3 set_a_ai_in_retail_and_e-commerce/logic/swarm.py
```

Open `http://localhost:4000` in your browser. Navigate to **Flows → ai_in_retail_and_e-commerce_agent_swarm** to run the flow and inspect each agent step in the trace viewer.

> Replace `set_a_ai_in_retail_and_e-commerce` with any of the other set directories (`set_b_ai_in_retail_and_e-commerce`, `set_c_supply_chain`, `set_d_edtech`, `set_e_legal`) to run those swarms.
