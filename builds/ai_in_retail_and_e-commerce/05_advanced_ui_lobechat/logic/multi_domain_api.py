...
and RAG retrievers (Session 04) across AI in Retail & E-commerce, AI in Retail & E-commerce, Supply Chain, EdTech, Legal.
...
LobeChat Plugin Endpoint:
    ...
    Body: { "domain": "ai_in_retail_and_e-commerce", "query": "What are the expense limits?" }
"""
...
DOMAIN_CONFIG = {
    "ai_in_retail_and_e-commerce": {
        "label": "Corporate AI in Retail & E-commerce",
        "swarm_module": "03_multi_agent_systems.set_a_ai_in_retail_and_e-commerce.logic.swarm",
        "swarm_flow": "ai_in_retail_and_e-commerce_agent_swarm",
        "rag_module": "04_sovereign_knowledge_rag.set_a_ai_in_retail_and_e-commerce.logic.ingest_and_query",
        "rag_flow": "ai_in_retail_and_e-commerce_rag_flow",
        "swarm_input_field": "csv_data",
        "session01_output": "/tmp/ai_in_retail_and_e-commerce_output/flagged_expenses.csv",
    },
    "ai_in_retail_and_e-commerce": {
        "label": "AI in Retail & E-commerce & HIPAA",
        "swarm_module": "03_multi_agent_systems.set_b_ai_in_retail_and_e-commerce.logic.swarm",
        "swarm_flow": "ai_in_retail_and_e-commerce_agent_swarm",
        "rag_module": "04_sovereign_knowledge_rag.set_b_ai_in_retail_and_e-commerce.logic.ingest_and_query",
        "rag_flow": "ai_in_retail_and_e-commerce_rag_flow",
        "swarm_input_field": "csv_data",
        "session01_output": "/tmp/ai_in_retail_and_e-commerce_output/scrubbed_billing.csv",
    },
    "supply_chain": {
...
# We'll use a robust fallback to map any span containing a domain keyword to that domain's folder
TRACE_DOMAIN_MAP = {}
TRACE_FILE_MAP = {}
    ...
    def export(self, spans: list[ReadableSpan]) -> SpanExportResult:
        try:
            ...
            for span in spans:
                ...
                # Retrieve domain from cache, or try to glean it from the span data
                domain = TRACE_DOMAIN_MAP.get(trace_id, "unknown")
                if domain == "unknown":
                    span_str = json.dumps(data).lower()
                    for d in ["ai_in_retail_and_e-commerce", "ai_in_retail_and_e-commerce", "supply_chain", "edtech", "legal"]:
                        # Look for domain signatures in the raw trace data (like "ai_in_retail_and_e-commerce_agent_swarm")
                        if f"{d}_" in span_str or f"domain: {d}" in span_str:
                            domain = d
...
class ChatRequest(BaseModel):
    domain: str = Field(
        description="Industry domain: ai_in_retail_and_e-commerce | ai_in_retail_and_e-commerce | supply_chain | edtech | legal",
        examples=["ai_in_retail_and_e-commerce"],
    )
    query: str = Field(
...
async def run_swarm(domain_key: str) -> str:
    ...
    # Run the flow within the preserved context to ensure TraceExporter can access it
    ctx = contextvars.copy_context()
    
    async def _execute():
        payload = {cfg["swarm_input_field"]: raw_data, "domain": domain_key}
        return await flow_fn(SwarmInput(**payload))
        
    result = await ctx.run(_execute)
