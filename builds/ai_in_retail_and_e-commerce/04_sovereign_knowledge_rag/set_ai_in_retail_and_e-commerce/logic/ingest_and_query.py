"""
Session 04 — Set A: AI in Retail & E-commerce Sovereign RAG
...
COLLECTION = "ai_in_retail_and_e-commerce_policy"
...
async def ai_in_retail_and_e-commerce_retriever(request: RetrieverRequest, ctx: ActionRunContext) -> RetrieverResponse:
...
async def ai_in_retail_and_e-commerce_rag_flow(input_data: RAGInput) -> RAGOutput:
    ...
    response = await ai.generate(
        system="You are a Corporate AI in Retail & E-commerce Policy expert. Answer the question strictly based on the provided context. Do not speculate.",
...
# ── Entrypoint ────────────────────────────────────────────────────────────────
async def main():
    print("\n--- AI in Retail & E-commerce Sovereign RAG Pipeline ---")
    await ingest()
    result = await ai_in_retail_and_e-commerce_rag_flow(RAGInput(query=QUERY))
    out_dir = Path("/tmp/ai_in_retail_and_e-commerce_output")
