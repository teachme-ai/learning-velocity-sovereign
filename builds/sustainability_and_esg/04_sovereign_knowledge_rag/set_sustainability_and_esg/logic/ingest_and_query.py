"""
Session 04 — Set A: Sustainability & ESG Sovereign RAG
...
COLLECTION = "sustainability_and_esg_policy"
...
async def sustainability_and_esg_retriever(request: RetrieverRequest, ctx: ActionRunContext) -> RetrieverResponse:
...
async def sustainability_and_esg_rag_flow(input_data: RAGInput) -> RAGOutput:
    ...
    response = await ai.generate(
        system="You are a Corporate Sustainability & ESG Policy expert. Answer the question strictly based on the provided context. Do not speculate.",
...
# ── Entrypoint ────────────────────────────────────────────────────────────────
async def main():
    print("\n--- Sustainability & ESG Sovereign RAG Pipeline ---")
    await ingest()
    result = await sustainability_and_esg_rag_flow(RAGInput(query=QUERY))
    out_dir = Path("/tmp/sustainability_and_esg_output")
