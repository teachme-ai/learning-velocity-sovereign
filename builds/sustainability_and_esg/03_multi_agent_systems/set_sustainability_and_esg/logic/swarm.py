"""
Session 03 — Set A: Sustainability & ESG Multi-Agent Swarm
...
# ── Genkit Flow ───────────────────────────────────────────────────────────────
@ai.flow()
async def sustainability_and_esg_agent_swarm(input_data: SwarmInput) -> SwarmOutput:
    print("Starting Corporate Sustainability & ESG Triple-Agent Swarm...")
...
async def main():
    input_file = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../../01_data_pipeline_automation/set_a_sustainability_and_esg/data/flagged_expenses.csv")
    )
    ...
    result = await sustainability_and_esg_agent_swarm(SwarmInput(csv_data=csv_data))

    output_dir = "/tmp/sustainability_and_esg_output"
